...
(function () {
  ...
  if (window.ConsentManagerInitialized) {
    console.log('ConsentManager already initialized, skipping...');
    return;
  }
  ...
  const ConsentManager = {
    ...
    activateForcedScripts() {
      ...
      console.log(`Activated ${activatedCount} forced scripts`);
    },
    ...
    setupDialogEventHandlers() {
      console.log("Setting up consent dialog event handlers");
      ...
      console.log('Dialog backdrop click handlers set up');
    },
    ...
    setupEventListeners() {
      console.log("Setting up consent event listeners");
      ...
      console.log("Consent event listeners set up successfully");
    },

    // Show the preferences panel with up-to-date information
    showPreferencesPanel() {
      console.log("Showing preferences panel");
    ...
    showDialogPreferences() {
      ...
      panels.forEach(panel => {
        ...
        if (dialog) {
          dialog.showModal();
          console.log("Using dialog with showModal() for dialog banner type");
        } else {
    ...
    showHorizontalPreferences() {
      ...
      if (horizontalBanner) {
        ...
        preferencesInHorizontal.forEach(panel => {
          this.resetWebflowFormIssues(panel);
          DOMUtils.showElement(panel);
          console.log("Showing preferences panel directly within horizontal banner");
        });
      } else {
        console.warn("No horizontal banner found, falling back to default behavior");
    ...
    hidePreferencesPanel() {
      console.log("Hiding preferences panel");
    ...
    hideDialogPreferences() {
      ...
      panels.forEach(panel => {
        ...
        if (dialog) {
          dialog.close();
          console.log("Dialog closed with native close()");
        } else {
    ...
    hideHorizontalPreferences() {
      ...
      if (horizontalBanner) {
        ...
        console.log("Hiding preferences panel within horizontal banner");
      } else {
        // Fallback
        DOMUtils.getElements('[data-consent-preferences]', false).forEach(panel => {
          DOMUtils.hideElement(panel);
    ...
    setConsent(preferences) {
      console.log("Setting consent preferences", preferences);
    ...
    updateTCFConsentIfAvailable(preferences) {
      if (window.__tcfapi) {
        window.__tcfapi('setConsent', 2, () => {
          console.log('TCF consent preferences updated');
        }, {
          'purpose': {
    ...
    activateScriptsForCategory(category) {
      ...
      console.log(`Activated ${activatedCount} scripts for category: ${category}`);
    },
